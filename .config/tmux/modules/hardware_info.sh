...
MEMORY_INFO="󰓅 Memory usage: ${MEMORY_USED}/${MEMORY_TOTAL} (${MEMORY_USED_PERCENTAGE}%%)"

CPU_LOAD=$(top -b -n1 | grep "Cpu(s)" | awk '{printf "%4.1f", $2 + $3 + $4}' )
CPU_TEMP=$(cat /sys/class/thermal/thermal_zone0/temp | awk '{ printf $0 / 1000 }')
CPU_INFO=" CPU load: ${CPU_LOAD}%%  ${CPU_TEMP}°C"
